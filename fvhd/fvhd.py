...
class FVHD(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        ...
        Args:
            ...
            mutual_neighbors_epochs (Optional[int]): Epochs to use mutual neighbor graph.
    ...
    def fit_transform(self, X: Union[np.ndarray, torch.Tensor], y=None, 
                      ...
                      mutual_idx: Optional[np.ndarray] = None,
                      graph: Optional[Graph] = None) -> np.ndarray:
        """
        ...
        Args:
            X: Input data.
            y: Ignored.
            nn_idx (Optional[np.ndarray]): Precomputed nearest neighbor indices.
            rn_idx (Optional[np.ndarray]): Precomputed random neighbor indices.
            mutual_idx (Optional[np.ndarray]): Precomputed mutual neighbor indices.
            graph (Optional[Graph]): Graph object containing nearest neighbors.
    ...
    def _compute_graphs(self, X: np.ndarray, precomputed_nn: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    def _force_directed_method(
        ...
        momentum-based update rule with velocity limiting and auto-adaptive learning rate.
